...
function buildCommentTree(
  ...
  map.forEach((item) => {
    if (item.comment_id && item.comment_id !== rootId && map.has(item.comment_id)) {
...
export default function PostComment({ postId }: PostCommentProps) {
  const [comments, setComments] = useState<(CommentVO & { replies?: CommentVO[] })[]>([])
  ...
  const [avatar, setAvatar] = useState<string>('https://weavatar.com/avatar/default')
  ...
  // 渲染评论及其回复
  const renderComments = (list: (CommentVO & { replies?: CommentVO[] })[], depth = 0) =>
    list.map((c) => {
      return (
        <div
          key={c.id}
          className={`flex gap-3 items-start py-4 group relative rounded-xl hover:bg-muted/40 transition-colors`}
            ...
            {c.url ? (
              <a href={c.url} target="_blank" rel="noopener noreferrer" className="shrink-0">
                <Avatar
                  ...
                  alt={c.nickname}
                  size="sm"
                  className="mt-1 transition-opacity hover:opacity-80 ring-2 ring-primary/30"
                />
              </a>
            ) : (
              <Avatar
                ...
                alt={c.nickname}
                size="sm"
                className="mt-1 ring-2 ring-primary/10"
              />
            )}
          </div>
          <div className="flex-1 min-w-0 z-10">
            <div className="flex items-center gap-2 mb-1">
              {c.url ? (
                ...
                <span className="font-semibold text-base text-primary">{c.nickname}</span>
              )}
              <span className="text-xs text-muted-foreground ml-1 font-normal">
                {dayjs(c.created_at).format('YY-MM-DD HH:mm:ss')}
              </span>
            </div>
            <div className="text-foreground break-words leading-relaxed bg-muted/30 rounded px-3 py-2">
              {c.content}
            </div>
            <div className="mt-2 flex items-center gap-2">
              <button
                className="p-0 text-xs font-medium flex items-center gap-1 text-primary/80 hover:text-primary transition-colors"
                onClick={() => {
                  setReplyTo(c.id)
            ...
            {c.replies && c.replies.length > 0 && (
              <div className="mt-3 space-y-2 relative">{renderComments(c.replies, depth + 1)}</div>
            )}
          </div>
        </div>
      )
  ...
  return (
    <div className="mx-auto space-y-8 max-w-2xl">
      <div ref={formRef} className="rounded-2xl bg-card/90 p-8 shadow-lg border border-border/60">
        <div className="flex flex-col gap-6 mb-4">
          ...
          {replyQuote && (
            <div className="mb-2 px-3 py-2 rounded border-l-4 border-primary/60 bg-muted/40 text-xs text-muted-foreground relative flex items-center gap-2">
              <span className="font-semibold text-primary">@{replyQuote.nickname}</span>
              ...
              <Button
                size="sm"
                variant="light"
                className="ml-auto px-1 py-0 h-6 text-xs flex items-center"
                onPress={() => {
                  setReplyTo('')
                  setReplyingName('')
                  setReplyQuote(null)
        ...
        <div className="flex justify-between items-center mt-2">
          {replyTo && !replyQuote && (
            <span className="text-xs text-muted-foreground">
              正在回复 <span className="text-primary font-semibold">@{replyingName}</span>
            </span>
          )}
          <div className="flex gap-2 ml-auto">
            ...
            <Button
              ...
              disabled={submitting}
              className="px-8 font-semibold shadow"
            >
              {replyTo ? '回复' : '发表评论'}
            </Button>
          </div>
      ...
      <div className="space-y-0 relative">
        {comments.length === 0 ? (
          <div className="text-center text-muted-foreground py-12 text-base tracking-wide">
            暂无评论，快来抢沙发吧！
          </div>
        ) : (
